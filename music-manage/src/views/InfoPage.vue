<template>
  ...
  <el-row :gutter="20">
    <el-col :span="12">
      <h3>歌曲收藏排名</h3>
      <el-card class="cav-info" shadow="hover" :body-style="{ padding: '0px' }" id="songCollectionRank"></el-card>
    </el-col>
    <el-col :span="12">
      <h3>歌手国籍</h3>
...
const songCollectionRank = reactive({
  xAxis: {
    type: "category",
    data:[],
    axisLabel: {
...
HttpManager.getCollectionOfSort().then(async(res) => {
  const result = res as ResponseBody;
  let i = 0;
  songCollect.value = result.data.length;

  
  for(let item of result.data){
    if(i < 10)
    {
      const songResult = (await HttpManager.getSongOfId(item.song_id)) as ResponseBody;
      const songName = songResult.data[0].name;
      console.log(songName);
      songCollectionRank.xAxis.data.push(songName);
      songCollectionRank.series[0].data[i] = item.count;
      i++;
    }
  }
 
  // 初始化并设置图表
  const songCollectionRankChart = echarts.init(document.getElementById("songCollectionRank"));
  songCollectionRankChart.setOption(songCollectionRank);
});
// ======================
